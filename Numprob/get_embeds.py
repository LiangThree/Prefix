...
def read_dataset(path):
    ...
    #     for key in full_dataset[spl].keys():
    #         if spl == 'train':
    #             full_dataset[spl][key] = full_dataset[spl][key][:720]
    #         else:
    #             full_dataset[spl][key] = full_dataset[spl][key][:90]
...
def main(args):
    ...
    for spl in dataset:
        ...
        for example in tqdm(examples):
            ...
            start = False
            for token_index, one_token in enumerate(tokens):
                try:
                    int(one_token)
                    start = True
                except ValueError:
                    if start:
                        special_positions.append(token_index)
            ...
            try:
                js['predicted'] = int(decoded[len(prompt):])
            except:
                js['predicted'] = example['golden']
                error_predict_count+=1
        ...
        print('Error prediction count: ', error_predict_count)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', type=str, default='dataset/cal_raw/raw.jsonl')
    parser.add_argument('--output_path', type=str, default='Numprob/embeddings')
