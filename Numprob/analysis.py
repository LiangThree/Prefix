...
def analyze_overall_accuracy(args, test_dataset):
    ...
    for index, row in test_dataset.iterrows():
        digit, golden, prediction = row['digit'], row['golden'], row['prediction']
        total[digit] = total.get(digit, 0)+1
        if (str(golden) in prediction):
...
def analyze_all_layers(
    ...
    for layer in tqdm(range(args.layer_count)):
        ...
        for j, X in enumerate(all_X):
            ...
            key_indices = []
            inputs = tokenizer(test_dataset['prompt'][j], return_tensors="pt")
    ...
    df.to_csv(f"{out_path}/acc.csv", index=False)
    fig = sns.heatmap(data=converted_df, annot=False, cmap='Blues')
    beautify(fig)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--target', type=str, choices=['a', 'b', 'golden', 'all'], required=True)
